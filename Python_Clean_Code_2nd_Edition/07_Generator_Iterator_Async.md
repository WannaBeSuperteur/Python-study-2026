...
* [5. 비동기 프로그래밍 관련](#5-비동기-프로그래밍-관련)
  * [5-1. 비동기 매직 메서드](#5-1-비동기-매직-메서드)
...
* Python에서는 다음이 가능하다.
  * 여러 코루틴을 특정 순서로 실행하도록 스케줄링
  * 일시 정지된 ```yield from``` 코루틴과의 데이터 통신

### 5-1. 비동기 매직 메서드

* 비동기 프로그래밍 관련 매직 메서드는 다음과 같다.
  * 참고: [컨텍스트 관리자의 magic method ```__enter__```, ```__exit__```](02_Pythonic_Code.md#1-컨텍스트-관리자-context-manager)

| 매직 메서드           | 설명                                                                                                     |
|------------------|--------------------------------------------------------------------------------------------------------|
| ```__aenter__``` | 비동기 컨텍스트 진입                                                                                            |
| ```__aexit__```  | 비동기 컨텍스트 종료                                                                                            |
| ```__aiter__```  | 객체가 비동기 이터레이터임을 나타내기 위해 사용                                                                             |
| ```__anext__```  | 코루틴에서는 이터레이터에서 사용하는 ```__next__``` (```next(iterator)``` 호출 시 작동하는) 사용이 불가능하므로, ```__anext__``` 코루틴 사용 |
