...
* [4. 디스크립터 사용의 가치](#4-디스크립터-사용의-가치)
* [5. 디스크립터 관련 이슈](#5-디스크립터-관련-이슈)
* [6. 디스크립터 관련 기타 사항](#6-디스크립터-관련-기타-사항)
...
* 디스크립터를 사용했을 때, 디스크립터 클래스 자체의 코드는 디스크립터를 사용하지 않고 setter 와 같은 메서드를 사용했을 때에 비해 다소 복잡하다.
* 그러나, **클라이언트 클래스의 코드를 간결하게 만들 수 있다** 는 점에서 가치가 있다.
...
| 디스크립터 관련 이슈 | 설명                                                                                                                                                                                    |
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 전역 상태 공유 이슈 | 클래스 본문에 정의되는 **클래스 속성 (class attribute)** 이 **모든 instance 에서 공유됨**<br>- 이로 인해, **디스크립터 객체를 통해 (공유되는) 데이터 보관** 시 문제가 발생할 수 있음<br>- 해결 방법: 항상 **인스턴스의 ```__dict__``` 에 데이터를 저장/반환** 한다. |
| 코드 재사용      | 같은 코드가 3회 이상 필요한 경우 재사용성 고려 (3의 규칙)                                                                                                                                                   |

## 6. 디스크립터 관련 기타 사항

* 어떤 디스크립터가 '좋은 디스크립터'인지를 판단하는 기준은 다음과 같다.
  * 파이썬 자체의 디스크립터와 유사함 (함수 등)
    * ```@property```, ```@classmethod``` ```@staticmethod``` 데코레이터는 디스크립터에 해당 (참고: PYDESCR-02 공식 문서)
* 데코레이터를 디스크립터로 구현할 수 있다.
  * 예를 들어, 데코레이터 클래스에 ```__get__``` 메서드를 구현할 수 있다.
* 디스크립터에는 **불필요한 메서드를 구현하지 않아야** 한다.
  * 즉, ```__get__``` ```__set__``` ```__delete__``` ```__set_name__``` 중 일부만 구현할 수 있다. 
* [SOLID 원칙](04_SOLID_Rule.md) 중 [인터페이스 분리 원칙](04_SOLID_Rule.md#5-인터페이스-분리-원칙-interface-segregation) (인터페이스를 작게 유지하라) 은 **디스크립터에도 적용된다.**
