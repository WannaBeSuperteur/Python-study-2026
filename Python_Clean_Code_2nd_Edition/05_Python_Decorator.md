...
* [4. 데코레이터를 사용하기 적합한 경우](#4-데코레이터를-사용하기-적합한-경우)
* [5. 데코레이터 사용 시의 실수](#5-데코레이터-사용-시의-실수)
* [6. 데코레이터 부작용의 활용](#6-데코레이터-부작용의-활용)
* [7. 데코레이터와 클린 코드](#7-데코레이터와-클린-코드)
  * [7-1. 데코레이터와 관심사의 분리](#7-1-데코레이터와-관심사의-분리)
  * [7-2. 좋은 데코레이터의 특징](#7-2-좋은-데코레이터의-특징)
...
데코레이터의 부작용을 활용하는 경우는 다음과 같다.

* import 시 ```@wraps``` 의 윗부분이 자동 실행된다는 점을 이용하여, 공용 레지스트리 객체 등록 등을 하는 경우

## 7. 데코레이터와 클린 코드

데코레이터를 활용한 클린 코드를 만드는 방법은 다음과 같다.

* 상속 (class inheritance) 보다는 **컴포지션 (composition, 다른 클래스를 가져와서 수정 후, 이를 새로운 객체로 반환)** 을 사용한다.
  * 데코레이터 자체가 이미 컴포지션 컨셉이다.
* 상속은 새로운 클래스가 진짜로 기존 클래스의 **하위 관계** 인 경우에 적합하다. (코드 재사용을 위한 '정답'이 아니다.)
* 확장성을 고려한다.
  * 파라미터에 따라 여러 가지 기능을 갖도록 **유연성 있게** 만들 수 있다.
* 여러 객체에 데코레이터를 적용함으로써, **DRY (Don't repeat yourself, 코드 반복 금지) 원칙** 을 충실히 준수할 수 있다.

결론적으로, 일반적으로 데코레이터를 사용하여 클린 코드를 만들 수 있는 경우는 다음과 같다.

* **패턴 발생 + 데코레이터 추상화 도출** 시에, 데코레이터를 사용하도록 리팩토링 실시
* 데코레이터 **3회 이상** 필요 시 구현
* 데코레이터 코드는 **최소한으로 유지**

### 7-1. 데코레이터와 관심사의 분리

* 데코레이터를 위해 사용하는 ```wrapped(*args, **kwargs)``` 함수에도 **단일 책임 원칙** 이 필요하다.
  * 즉 ```wrapped``` 함수가 여러 개의 책임을 진다면, 2개 이상의 데코레이터로 분리한 후 다음과 같이 해당 데코레이터들을 적용할 수 있다.
  * 이때, **데코레이터의 적용 순서** 역시 중요하다.
...
좋은 데코레이터가 갖춰야 하는 특징은 다음과 같다.

* 캡슐화와 관심사의 분리 (실제로 하는 일 - 데코레이팅하는 일 간의 **책임을 명확히 구분** 한다.)
* 독립성 (데코레이터는 데코레이팅되는 객체와 **최대한 분리** 한다.)
* 재사용성 (데코레이터는 **여러 개의 함수에 적용 가능** 해야 한다.)
