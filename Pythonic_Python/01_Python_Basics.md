...
* [3. 파이썬 모듈, 패키지 관련](#3-파이썬-모듈-패키지-관련)
...
| 구분            | 설명                                               |
|---------------|--------------------------------------------------|
| 모듈 (module)   | ```.py``` 파일이 하나의 모듈에 해당                         |
| 패키지 (package) | 모듈을 모은 디렉토리<br>- 패키지로 인식되려면 ```__init__.py``` 필요 |

* ```__init__.py``` 에서 다음과 같이 ```__all__``` 을 이용하여 **외부 공개 대상 모듈을 명시** 할 수 있다.

```python
# init.py

__all__ = ["run", "utils", "deep_learning_model"]
```
